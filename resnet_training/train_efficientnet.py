...
def img2label_paths(img_paths):
    ...
    sa, sb = 'crop_img_new' + os.sep, 'crop_label_new' + os.sep  # crop_img/, crop_label/ substrings
...
class TextDataset(Dataset):
    def __init__(self, imgs_path, preprocessed=False, img_size = 64):
    ...
    def __getitem__(self, idx):
        ...
        if self.preprocessed:
            ...
            img = cv2.bitwise_not(morph2, morph2)
...
class TextModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.extract._fc = nn.Linear(2560, 5038)
...
if __name__ == '__main__':
    ...
    # create dataset
    if valid:
        train_set = TextDataset(os.path.normcase('new_crop_train.txt'), False, img_size)
        val_set = TextDataset(os.path.normcase('new_crop_val.txt'), False, img_size)
    else:
        train_set = TextDataset(os.path.normcase('new_crop_total.txt'), False, img_size)
    # create dataloader
    ...
    '''
    if valid:
        torch.save(model.state_dict(), 'eff_final_imgsz%d.pt' % img_size)
    else:
        torch.save(model.state_dict(), 'eff_final_no_val_imgsz%d.pt' % img_size)
